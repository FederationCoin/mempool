...
This directory contains Dockerfiles and a `docker-compose.yml` for the FederationCoin explorer fork. Images containerize the frontend and backend only. You still need `federationcoind -testnet` (and later an Electrum server for address pages).

Build from **this** tree. Do not use mempool.space or `ghcr.io/retropex` as the product image.

Default compose is **testnet**: `CORE_RPC_PORT=35332`, `MEMPOOL_NETWORK=testnet`, `MEMPOOL_BACKEND=none`. Dummy MAIN (RPC 4094) is not launched.

Jump to a section in this doc:
- [Configure with FederationCoin Core Only](#configure-with-federationcoin-core-only)
- [Configure with FederationCoin Core + Electrum Server](#configure-with-federationcoin-core--electrum-server)
- [Further Configuration](#further-configuration)

## Configure with FederationCoin Core Only

_Note: address lookups require an Electrum Server and will not work with this configuration._

The default Docker configuration assumes you have the following in your node config (testnet):
...
The IP address in the example above refers to Docker's default gateway IP address so that the container can hit `federationcoind` on the host. If your setup is different, update it accordingly.

Make sure `federationcoind -testnet` is running and synced.
...
Your instance should be running at http://localhost. The graphs will be populated as new transactions are detected.

## Configure with FederationCoin Core + Electrum Server

First, configure the node as specified above, and make sure your Electrum Server is running and synced.
...
With the node and Electrum Server set up, run:
